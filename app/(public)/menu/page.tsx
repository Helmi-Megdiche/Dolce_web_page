    ...
    <div className="mx-auto max-w-6xl px-4 py-12">
      <div className="mb-10 text-center">
        ...
        <p className="mt-3 text-dolce-text/70">
          Crêpes, pancakes, bubble waffles et douceurs
        </p>
      </div>
          ...
          <button
            ...
            className={`rounded-full px-4 py-2 text-sm font-medium transition ${
              active === cat
                ...
                : "bg-dolce-secondary text-dolce-text hover:bg-dolce-accent/40"
            }`}
          >
            {cat === "All" ? "Tout" : cat}
          </button>
      ...
      {loading && (
        <p className="py-20 text-center text-dolce-text/60">Chargement…</p>
      )}
      {error && <p className="py-20 text-center text-red-600">{error}</p>}

      {!loading && !error && filtered.length === 0 && (
        <p className="py-20 text-center text-dolce-text/60">
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {filtered.map((item) => (
          <article
            key={item._id}
            className="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-dolce-secondary/50 transition hover:shadow-md"
          >
            <div className="relative aspect-[4/3] bg-dolce-secondary">
              {item.imageUrl ? (
                <Image
                  src={item.imageUrl}
                  alt={item.name}
            ...
            <div className="p-5">
              <div className="flex items-start justify-between gap-3">
                ...
                <span className="shrink-0 font-semibold text-dolce-primary">
                  {item.price} DT
                </span>
              </div>
              {item.description && (
                <p className="mt-2 text-sm text-dolce-text/65">
                  {item.description}
                </p>
              )}
              <span className="mt-3 inline-block text-xs font-medium text-green-700">
                ● Disponible
              </span>
            </div>
          </article>
